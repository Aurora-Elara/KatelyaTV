...
export class LocalStorage implements IStorage {
  ...
  // ---------- 书城 ----------
  private readBookRecords<T>(
    prefix: string,
    userName: string
  ): Record<string, T> {
    if (typeof window === 'undefined') return {};
    const root = this.getStorageKey(prefix, userName);
    const result: Record<string, T> = {};
    for (let index = 0; index < localStorage.length; index += 1) {
      const storageKey = localStorage.key(index);
      if (!storageKey?.startsWith(`${root}_`)) continue;
      const value = localStorage.getItem(storageKey);
      if (value) result[storageKey.slice(root.length + 1)] = JSON.parse(value);
    }
    return result;
  }
  ...
  async getBookFavorites(
    ...
    return this.readBookRecords<BookFavorite>('book_fav', userName);
  }
  ...
  async setBookFavorite(
    ...
    localStorage.setItem(
      this.getStorageKey('book_fav', userName, key),
  ...
  async deleteBookFavorite(userName: string, key: string): Promise<void> {
    if (typeof window === 'undefined') return;
    localStorage.removeItem(this.getStorageKey('book_fav', userName, key));
  }
  ...
  async getBookProgress(
    ...
    const value = localStorage.getItem(
      this.getStorageKey('book_progress', userName, key)
    );
  ...
  async getAllBookProgress(
    ...
    return this.readBookRecords<BookProgress>('book_progress', userName);
  }
  ...
  async getBookBookmarks(
    ...
    return this.readBookRecords<BookBookmark>('book_bookmark', userName);
  }
  ...
  async setBookBookmark(
    ...
    if (typeof window === 'undefined') return;
    localStorage.setItem(
      this.getStorageKey('book_bookmark', userName, bookmark.id),
  ...
  async deleteBookBookmark(userName: string, id: string): Promise<void> {
    if (typeof window === 'undefined') return;
    localStorage.removeItem(this.getStorageKey('book_bookmark', userName, id));
  }
